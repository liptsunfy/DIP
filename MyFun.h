...
unsigned char *m_ruImgDataOut;  //最终解包裹结果
...
// 解包裹4
void PhaseUnwrap4(unsigned char *pImgData,int imgWidth, int BitCount, int imgHeight,int dPhaseDif)
{
    ...
    if(m_ruImgDataOut != NULL)
    {
        delete []m_ruImgDataOut;
        m_ruImgDataOut = NULL;
    }
    m_ruImgDataOut = new unsigned char[lineByteOut * m_imgHeightOut];
    ...
    for ( i = 0; i < m_imgHeightOut; i++)
    {
        for ( j = 0; j < m_imgWidthOut; j++)
        {
            m_ruImgDataOut[i * lineByteOut + j]
                = (unsigned char) ( (pBufData[i * imgWidth + j] - Min) * 255 / ( Max - Min ) );
        }
    }
